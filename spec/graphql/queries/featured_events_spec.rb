...
describe "Types::EventType" do
  describe "featured_events" do
    ...
    let!(:not_holding_event) { create(:event, start_at: Time.zone.now.since(1.day),  end_at: Time.zone.now.since(2.day)) }
    let!(:holding_event) { create(:event, start_at: Time.zone.now.ago(1.day), end_at: Time.zone.now.since(1.day)) }
